            ...
            @foreach($savedPasswords as $savedPassword)
                <li class="p-4 bg-gray-100 rounded-lg ">
                    ...
                    <div class="flex flex-col">
                        @if ($savedPassword->description)
                        <span class="text-gray-500 text-sm">Description: {{ $savedPassword->description }}</span>
                        @endif
                        <span class="text-gray-500 text-sm">Type: {{ $savedPassword->type }}</span>
                        @if ($savedPassword->created_by != Auth::id())
                        <span class="text-gray-500 text-sm">{{ $savedPassword->creator->name }} Shared this with you.</span>
                        @endif
                    </div>
